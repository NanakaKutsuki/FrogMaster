...
public class HybridStrategy3 extends AbstractStrategy {
    ...
    public HybridStrategy3(Ticker ticker, TreeMap<LocalDateTime, Bar> barMap, BigDecimal bankrollBar) {
	...
	if (isLimit(bar)) {
	    ...
	    if (mom.compareTo(BigDecimal.valueOf(-8.75)) == -1 && accel.compareTo(input.getAccelST()) == -1) {
	...
	if (!initialized) {
	    ...
	    } else if (bar.getDateTime().toLocalTime().isAfter(LocalTime.of(7, 59))
		    && bar.getDateTime().toLocalTime().isBefore(START) && isStopLoss(bar)) {
		marketBuy = true;
	    }
	}
    }
